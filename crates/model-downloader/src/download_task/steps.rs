...
pub(super) async fn download<M: DownloadableModel>(
    ...
    progress_callback: impl Fn(poha_download_interface::DownloadProgress) + Send + Sync,
) -> Result<(), poha_file::Error> {
    poha_file::download_file_parallel_cancellable(
        &params.url,
        &params.destination,
        progress_callback,
...
pub(super) async fn verify_checksum<M: DownloadableModel>(
    ...
    let checksum_result = tokio::task::spawn_blocking(move || {
        poha_file::calculate_file_checksum(destination_for_checksum)
    })
    .await;
