...
fn log_download_error(error: &poha_file::Error) -> Option<String> {
    if matches!(error, poha_file::Error::Cancelled) {
        return None;
    }
    ...
    let reason = match error {
        poha_file::Error::ReqwestError(e) => {
            if e.is_timeout() {
        ...
        poha_file::Error::FileIOError(e) => {
            format!("File system error: {}", e)
        }
        poha_file::Error::Cancelled => unreachable!(),
        poha_file::Error::OtherError(msg) => msg.clone(),
    };
    Some(reason)
}
