...
use poha_audio_chunking::{AudioChunk, Chunker, SpeechChunker, SpeechChunkingConfig};
...
where
    E: From<poha_audio_chunking::Error>,
{
    let mut chunker =
...
pub fn split_resampled_channels(samples: &[f32], channel_count: usize) -> Vec<Vec<f32>> {
    ...
    poha_audio_utils::deinterleave(samples, channel_count)
}
...
#[cfg(test)]
mod tests {
    use poha_audio_chunking::{AudioChunk, Chunker};
    ...
    fn empty_audio_marks_channel_complete() {
        let chunks = chunk_channel_audio::<poha_audio_chunking::Error>(&[]).unwrap();
