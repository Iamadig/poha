...
use poha_audio_chunking::{SpeechChunkExt, SpeechChunkingConfig};
use poha_audio_interface::AsyncSource;
use poha_model_manager::{ModelManager, ModelManagerBuilder};
use poha_transcribe_core::TARGET_SAMPLE_RATE;
use poha_ws_utils::ConnectionManager;
use tokio::sync::mpsc;
use tower::Service;
...
pub struct TranscribeService {
    model_path: PathBuf,
    manager: ModelManager<poha_whisper_local::LoadedWhisper>,
...
async fn handle_websocket(
    ...
    guard: poha_ws_utils::ConnectionGuard,
    model: Arc<poha_whisper_local::LoadedWhisper>,
    manager: ModelManager<poha_whisper_local::LoadedWhisper>,
) {
    let (mut ws_sender, mut ws_receiver) = socket.split();
    let total_channels = (params.channels as usize).max(1);
    let redemption_time = redemption_time(&params);
    let languages: Vec<poha_whisper::Language> = params
        .languages
        .iter()
                        ...
                        match process_incoming_message(&message, params.channels.max(1)) {
                            ...
                            Ok(IncomingMessage::Audio(AudioExtract::Dual { ch0, ch1 })) => {
                                if total_channels >= 2 {
                                    ...
                                    let mixed = poha_audio_utils::mix_audio_f32(&ch0, &ch1);
...
fn build_transcription_streams(
    ...
    loaded_model: &poha_whisper_local::LoadedWhisper,
    languages: &[poha_whisper::Language],
...
struct TranscribeChannelStream<S> {
    channel_idx: usize,
    chunk_stream: S,
    model: poha_whisper_local::Whisper,
    pending: VecDeque<crate::service::Segment>,
}

impl<S> TranscribeChannelStream<S> {
    fn new(channel_idx: usize, chunk_stream: S, model: poha_whisper_local::Whisper) -> Self {
        Self {
            channel_idx,
            chunk_stream,
            model,
...
where
    S: Stream<Item = Result<poha_audio_chunking::AudioChunk, poha_audio_chunking::Error>> + Unpin,
{
