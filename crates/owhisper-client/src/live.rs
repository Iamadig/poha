...
use poha_ws_client::client::{
    ClientRequestBuilder, Message, Utf8Bytes, WebSocketClient, WebSocketHandle, WebSocketIO,
};

use crate::{
    DeepgramAdapter, RealtimeSttAdapter, append_provider_param, is_poha_proxy,
...
pub struct ListenClientBuilder<A: RealtimeSttAdapter = DeepgramAdapter> {
    ...
    pub(crate) connect_policy: Option<poha_ws_client::client::WebSocketConnectPolicy>,
...
impl<A: RealtimeSttAdapter> ListenClientBuilder<A> {
    ...
    pub fn connect_policy(
        mut self,
        policy: poha_ws_client::client::WebSocketConnectPolicy,
    ) -> Self {
    ...
    async fn build_request(
        ...
        params: &ListenParams,
        channels: u8,
    ) -> poha_ws_client::client::ClientRequestBuilder {
        ...
        let mut request = poha_ws_client::client::ClientRequestBuilder::new(uri);

        if is_poha_proxy(original_api_base) {
...
impl WebSocketIO for ListenClientIO {
    ...
    fn from_message(msg: Message) -> Result<Option<Self::Output>, poha_ws_client::Error> {
        Ok(match msg {
...
impl<A: RealtimeSttAdapter> ListenClient<A> {
    ...
    pub async fn from_realtime_audio(
        ...
        (
            impl Stream<Item = Result<StreamResponse, poha_ws_client::Error>>,
            SingleHandle,
        ),
        poha_ws_client::Error,
    > {
        ...
        let mapped_stream = raw_stream.flat_map(move |result| {
            ...
            let responses: Vec<Result<StreamResponse, poha_ws_client::Error>> = match result {
...
type DualOutputStream =
    Pin<Box<dyn Stream<Item = Result<StreamResponse, poha_ws_client::Error>> + Send>>;
...
impl<A: RealtimeSttAdapter> ListenClientDual<A> {
    ...
    ) -> Result<(DualOutputStream, DualHandle), poha_ws_client::Error> {
...
fn merge_streams_with_channel_remap<S1, S2>(
    mic_stream: S1,
    spk_stream: S2,
) -> impl Stream<Item = Result<StreamResponse, poha_ws_client::Error>> + Send
where
    S1: Stream<Item = Result<StreamResponse, poha_ws_client::Error>> + Send + 'static,
    S2: Stream<Item = Result<StreamResponse, poha_ws_client::Error>> + Send + 'static,
{
...
fn websocket_client_with_keep_alive<A: RealtimeSttAdapter>(
    ...
    adapter: &A,
    connect_policy: Option<poha_ws_client::client::WebSocketConnectPolicy>,
) -> WebSocketClient {
    ...
    async fn test_proxy_deepgram_single() {
        let client = ListenClient::builder()
            ...
            .params(owhisper_interface::ListenParams {
                ...
                languages: vec![poha_language::ISO639::En.into()],
                ..Default::default()
            })
            .build_single()
            .await;
