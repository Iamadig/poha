...
impl SmallestAIAdapter {
    ...
    fn is_language_supported(language: &poha_language::Language) -> bool {
    ...
    pub(crate) fn language_query_value(languages: &[poha_language::Language]) -> String {
        match languages {
...
mod tests {
    ...
    fn test_build_ws_url_from_base_proxy() {
        let (url, params) = SmallestAIAdapter::build_ws_url_from_base(
            "https://api.poha.invalid/stt?provider=smallestai",
        );
        assert_eq!(url.as_str(), "wss://api.poha.invalid/stt/listen");
        assert_eq!(
            params,
