...
impl MistralAdapter {
    fn is_language_supported(lang: &poha_language::Language) -> bool {
        lang.matches_any_code(SUPPORTED_LANGUAGES)
    }

    fn language_support_impl(languages: &[poha_language::Language]) -> LanguageSupport {
    ...
    pub fn language_support_live(languages: &[poha_language::Language]) -> LanguageSupport {
        Self::language_support_impl(languages)
    }

    pub fn language_support_batch(languages: &[poha_language::Language]) -> LanguageSupport {
        Self::language_support_impl(languages)
    }

    pub fn is_supported_languages_live(languages: &[poha_language::Language]) -> bool {
        Self::language_support_live(languages).is_supported()
    }

    pub fn is_supported_languages_batch(languages: &[poha_language::Language]) -> bool {
...
mod tests {
    ...
    fn test_build_ws_url_from_base_proxy() {
        let (url, params) =
            MistralAdapter::build_ws_url_from_base("https://api.poha.invalid?provider=mistral");
        assert_eq!(url.as_str(), "wss://api.poha.invalid/listen");
        assert_eq!(
            params,
