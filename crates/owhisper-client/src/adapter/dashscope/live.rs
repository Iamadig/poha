...
impl RealtimeSttAdapter for DashScopeAdapter {
    ...
    fn initial_message(
        ...
        tracing::debug!(
            poha.payload.size_bytes = json.len() as u64,
    ...
    fn parse_response(&self, raw: &str) -> Vec<StreamResponse> {
        ...
        match event {
            DashScopeEvent::SessionCreated { session } => {
                tracing::debug!(
                    poha.stt.provider_session.id = %session.id,
            ...
            DashScopeEvent::InputAudioBufferSpeechStarted { item_id } => {
                tracing::debug!(poha.stt.item.id = %item_id, "dashscope_speech_started");
                vec![]
            }
            DashScopeEvent::InputAudioBufferSpeechStopped { item_id } => {
                tracing::debug!(poha.stt.item.id = %item_id, "dashscope_speech_stopped");
                vec![]
            }
            DashScopeEvent::ConversationItemInputAudioTranscriptionCompleted {
                ...
                tracing::debug!(
                    poha.stt.item.id = %item_id,
                    poha.transcript.char_count = transcript.chars().count() as u64,
            ...
            DashScopeEvent::ConversationItemInputAudioTranscriptionText {
                ...
                tracing::debug!(
                    poha.stt.item.id = %item_id,
                    poha.transcript.char_count = text.chars().count() as u64,
