...
impl SonioxAdapter {
    async fn do_transcribe_file(
        ...
        tracing::info!(poha.file.path = %file_path.display(), "uploading_file_to_soniox");
        let file_id = soniox::upload_file(&client, &file_name, file_bytes, api_key)
            .await
            .map_err(soniox_err)?;

        tracing::info!(poha.file.id = %file_id, "soniox_file_uploaded");
    ...
    async fn transcribe_and_fetch(
        ...
        let transcription_id = soniox::create_transcription(client, &body, api_key)
            ...
            .map_err(soniox_err)?;
        tracing::info!(
            poha.stt.job.id = %transcription_id,
...
mod tests {
    ...
    async fn test_soniox_batch_transcription() {
        ...
        let audio_path = std::path::PathBuf::from(poha_data::english_1::AUDIO_PATH);
