...
use poha_ws_client::client::Message;
...
pub trait RealtimeSttAdapter: Clone + Default + Send + Sync + 'static {
    ...
    fn is_supported_languages(
        &self,
        languages: &[poha_language::Language],
        model: Option<&str>,
    ) -> bool;
...
fn is_poha_cloud(base_url: &str) -> bool {
    host_matches(base_url, |h| {
        h.contains("poha.invalid") || h.contains("poha.local")
    })
}

fn is_poha_local_proxy(base_url: &str) -> bool {
    url::Url::parse(base_url)
        .ok()
...
pub fn is_poha_proxy(base_url: &str) -> bool {
    is_poha_cloud(base_url) || is_poha_local_proxy(base_url)
}

pub fn normalize_languages(languages: &[poha_language::Language]) -> Vec<poha_language::Language> {
...
fn is_local_argmax(base_url: &str) -> bool {
    host_matches(base_url, is_local_host) && !is_poha_local_proxy(base_url)
}

fn is_cactus_model(model: &str) -> bool {
    model.parse::<poha_cactus_model::CactusSttModel>().is_ok()
}
...
pub fn build_proxy_ws_url(api_base: &str) -> Option<(url::Url, Vec<(String, String)>)> {
    ...
    if !host.contains("poha.invalid") && !host.contains("poha.local") && !is_local_host(host) {
        return None;
    }
...
impl AdapterKind {
    ...
    pub fn language_support_live(
        ...
        match self {
            ...
            Self::Poha | Self::Cactus => LanguageSupport::Supported {
...
mod tests {
    ...
    fn test_normalize_languages_deduplicates_same_base() {
        use poha_language::{ISO639, Language};
    ...
    #[test]
    fn test_is_poha_proxy() {
        assert!(is_poha_proxy("https://api.poha.invalid/stt"));
        assert!(is_poha_proxy("https://api.poha.invalid"));
        assert!(is_poha_proxy("https://api.poha.local/stt"));
        assert!(is_poha_proxy("https://api.poha.local"));
        assert!(is_poha_proxy("http://localhost:3001/stt"));
        assert!(is_poha_proxy("http://127.0.0.1:3001/stt"));

        assert!(!is_poha_proxy("https://api.deepgram.com"));
        assert!(!is_poha_proxy("http://localhost:50060/v1"));
    }
    ...
    fn test_is_local_argmax() {
        ...
        assert!(!is_local_argmax("https://api.poha.invalid/stt"));
    ...
    fn test_adapter_kind_from_url_and_languages() {
        use poha_language::ISO639::*;

        let cases: &[(&str, &[poha_language::ISO639], Option<&str>, AdapterKind)] = &[
            // PohaCloud - always routes to Poha adapter (proxy owns provider selection)
            (
                "https://api.poha.invalid/stt",
                &[En],
                None,
                AdapterKind::Poha,
            ),
            ...
            // localhost proxy
            ("http://localhost:3001/stt", &[En], None, AdapterKind::Poha),
            ("http://localhost:3001/stt", &[Ar], None, AdapterKind::Poha),
            // localhost argmax
            (
        ...
        for (url, langs, model, expected) in cases {
            let langs: Vec<poha_language::Language> = langs.iter().map(|l| (*l).into()).collect();
            assert_eq!(
    ...
    fn test_build_proxy_ws_url() {
        let cases: &[(&str, Option<(&str, Vec<(&str, &str)>)>)] = &[
            ...
            (
                "https://api.poha.invalid/stt?provider=soniox",
                Some((
                    "wss://api.poha.invalid/stt/listen",
                    vec![("provider", "soniox")],
                )),
            ),
            (
                "https://api.poha.invalid/stt/listen?provider=deepgram",
                Some((
                    "wss://api.poha.invalid/stt/listen",
            ...
            (
                "https://api.poha.invalid/stt/some/path?provider=fireworks",
                Some((
                    "wss://api.poha.invalid/stt/some/path/listen",
    ...
    #[test]
    fn test_poha_proxy_always_selects_poha_adapter() {
        use poha_language::ISO639::*;

        let proxy_urls = &[
            "https://api.poha.invalid/stt",
            "https://api.poha.local/stt",
            "http://localhost:3001/stt",
            "http://127.0.0.1:3001/stt",
        ];

        let language_combos: &[&[poha_language::ISO639]] =
            &[&[En], &[Ko], &[En, De], &[En, Ko], &[Ar]];

        for url in proxy_urls {
            for langs in language_combos {
                let langs: Vec<poha_language::Language> =
                    langs.iter().map(|l| (*l).into()).collect();
                assert_eq!(
                    AdapterKind::from_url_and_languages(url, &langs, Some("cloud")),
                    AdapterKind::Poha,
                    "proxy URL should always select Poha adapter regardless of languages: url={url}, langs={langs:?}"
                );
            }
        }
    }
    ...
    #[test]
    fn test_poha_adapter_supports_all_languages() {
        use poha_language::ISO639::*;

        let combos: &[&[poha_language::ISO639]] =
            &[&[En], &[Ko], &[Ar], &[En, De], &[En, Ko], &[Zh]];

        for langs in combos {
            let langs: Vec<poha_language::Language> = langs.iter().map(|l| (*l).into()).collect();
            assert!(
                AdapterKind::Poha.is_supported_languages_live(&langs, Some("cloud")),
                "Poha adapter should support all languages: {langs:?}"
            );
        }
    }
    ...
    fn test_direct_provider_urls_not_affected() {
        ...
        let en: Vec<poha_language::Language> = vec![En.into()];
        assert_eq!(
    ...
    fn test_append_provider_param_replaces_existing() {
        let url = append_provider_param("https://api.poha.invalid/stt?provider=deepgram", "poha");
        assert!(
            url.contains("provider=poha"),
    ...
    fn test_append_provider_param_preserves_other_params() {
        let url = append_provider_param(
            "https://api.poha.invalid/stt?model=cloud&provider=soniox&language=en",
            "poha",
        );
        assert!(
        ...
        assert!(url.contains("provider=poha"));
        assert!(!url.contains("provider=soniox"));
    }

    #[test]
    fn test_append_provider_param_no_existing_provider() {
        let url = append_provider_param("https://api.poha.invalid/stt", "poha");
        assert!(url.contains("provider=poha"));
