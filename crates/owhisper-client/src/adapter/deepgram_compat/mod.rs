...
mod tests {
    ...
    fn test_listen_endpoint_url_preserves_query_params() {
        let (url, params) = listen_endpoint_url("https://api.poha.invalid/v1?provider=deepgram");
        assert_eq!(url.as_str(), "https://api.poha.invalid/v1/listen");
    ...
    fn test_listen_endpoint_url_no_double_listen() {
        let (url, params) =
            listen_endpoint_url("https://api.poha.invalid/listen?provider=deepgram");
        assert_eq!(url.as_str(), "https://api.poha.invalid/listen");
        assert_eq!(params, vec![("provider".into(), "deepgram".into())]);
    }

    #[test]
    fn test_listen_endpoint_url_no_double_listen_with_trailing_slash() {
        let (url, params) = listen_endpoint_url("https://api.poha.invalid/listen/");
        assert_eq!(url.as_str(), "https://api.poha.invalid/listen/");
        assert!(params.is_empty());
    }
