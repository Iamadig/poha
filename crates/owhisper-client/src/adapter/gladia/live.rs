        ...
        async move {
            ...
            let (id, url) = match init {
                ...
                InitResponse::Error {
                    ...
                    tracing::error!(
                        error = %message,
                        poha.validation.errors = ?validation_errors,
                        "gladia_init_failed"
                    );
                    return None;
                }
    ...
    fn parse_response(&self, raw: &str) -> Vec<StreamResponse> {
        let msg: GladiaMessage = match serde_json::from_str(raw) {
            ...
            Err(e) => {
                tracing::warn!(
                    error = ?e,
                    poha.payload.size_bytes = raw.len() as u64,
        ...
        match msg {
            ...
            GladiaMessage::StartSession { id } => {
                tracing::debug!(poha.stt.provider_session.id = %id, "gladia_session_started");
                vec![]
            }
            GladiaMessage::EndSession { id } => {
                let channels = SessionChannels::remove(&id).unwrap_or_else(|| {
                    tracing::warn!(
                        poha.stt.provider_session.id = %id,
                ...
                tracing::debug!(
                    poha.stt.provider_session.id = %id,
                    poha.audio.channel_count = channels,
...
mod tests {
    ...
    fn test_build_language_config_three_languages() {
        let params = owhisper_interface::ListenParams {
            languages: vec![
                poha_language::ISO639::En.into(),
                poha_language::ISO639::Ko.into(),
                poha_language::ISO639::Ja.into(),
            ],
            ..Default::default()
        };
