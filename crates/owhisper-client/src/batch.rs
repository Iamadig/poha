...
use crate::adapter::{BatchSttAdapter, append_provider_param, is_poha_proxy};
...
mod tests {
    use super::*;
    use crate::{DeepgramAdapter, OpenAIAdapter, PohaAdapter};
    use poha_language::{ISO639, Language};

    #[test]
    fn injects_provider_for_poha_proxy() {
        let client = BatchClient::<PohaAdapter>::builder()
            .api_base("https://api.poha.invalid/stt")
            .api_key("test")
            .build();

        assert!(client.api_base.contains("provider=poha"));
    }
    ...
    #[test]
    fn injects_provider_for_direct_provider_adapter_on_poha_proxy() {
    ...
    fn rewrites_existing_provider_for_direct_provider_adapter() {
        let client = BatchClient::<OpenAIAdapter>::builder()
            .api_base("https://api.poha.invalid/stt?provider=poha&model=whisper-1")
            .api_key("test")
            .build();
