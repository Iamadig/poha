...
fn silero_onnx_mask(audio: &[u8]) -> (Vec<bool>, f32) {
    ...
    for (i, chunk) in samples_f32.chunks(CHUNK_SIZE_16KHZ).enumerate() {
        if chunk.len() == CHUNK_SIZE_16KHZ {
            let view = poha_onnx::ndarray::ArrayView1::from(chunk);
...
fn silero_cactus_mask(audio: &[u8], total_samples: usize) -> Vec<bool> {
    ...
    let path = format!(
        "{}/Library/Application Support/com.iamadig.poha.dev/models/cactus/whisper-medium-int8-apple/vad",
        home
    );
...
fn compare_all_engines() {
    compare("english_1", poha_data::english_1::AUDIO);
    compare("english_2", poha_data::english_2::AUDIO);
    compare("english_3", poha_data::english_3::AUDIO);
    compare("korean_1", poha_data::korean_1::AUDIO);
    compare("korean_2", poha_data::korean_2::AUDIO);
}
