use poha_onnx::ndarray::{Array3, ArrayView1};
use poha_onnx::ort::value::Tensor;
...
pub enum Error {
    ...
    InvalidInput(String),
    #[error("onnx error: {0}")]
    Onnx(#[from] poha_onnx::Error),
    #[error("ort error: {0}")]
    Ort(#[from] poha_onnx::ort::Error),
}

pub struct SileroVad {
    session: poha_onnx::ort::session::Session,
    state: Array3<f32>,
    context: Vec<f32>,
}
...
impl SileroVad {
    ...
    pub fn new(model_path: impl AsRef<std::path::Path>) -> Result<Self, Error> {
        let session = poha_onnx::load_model_from_path(model_path)?;
        Ok(Self {
            session,
    ...
    pub fn new_from_bytes(bytes: &[u8]) -> Result<Self, Error> {
        let session = poha_onnx::load_model_from_bytes(bytes)?;
        Ok(Self {
            session,
            state: Array3::zeros((2, 1, STATE_SIZE)),
            context: vec![0.0; CONTEXT_SIZE_16KHZ],
...
mod tests {
    ...
    use poha_onnx::ndarray::ArrayView1;
    ...
    fn test_silero_v6_english_1() {
        run_snapshot_test(poha_data::english_1::AUDIO, "silero_v6_english_1");
    }

    #[test]
    fn test_silero_v6_english_2() {
        run_snapshot_test(poha_data::english_2::AUDIO, "silero_v6_english_2");
    }

    #[test]
    fn test_silero_v6_english_3() {
        run_snapshot_test(poha_data::english_3::AUDIO, "silero_v6_english_3");
    }

    #[test]
    fn test_silero_v6_korean_1() {
        run_snapshot_test(poha_data::korean_1::AUDIO, "silero_v6_korean_1");
    }

    #[test]
    fn test_silero_v6_korean_2() {
        run_snapshot_test(poha_data::korean_2::AUDIO, "silero_v6_korean_2");
    }
}
