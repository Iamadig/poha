...
use poha_audio_utils::f32_to_i16_bytes;
use poha_vad_masking::VadMask;
...
impl Pipeline {
    ...
    fn dispatch(
        ...
        let recording_mic = Self::recording_mic(&frame, mode);
        let recording_processed_mic = Self::recording_processed_mic(&frame, mode);
        ...
        if let Some(actor) = recorder {
            let result = match mode {
                ChannelMode::MicOnly => actor.cast(RecMsg::AudioSingle(Arc::clone(&recording_mic))),
                ...
                ChannelMode::MicAndSpeaker => actor.cast(RecMsg::AudioDual {
                    raw_mic: Arc::clone(&recording_mic),
                    processed_mic: recording_processed_mic.as_ref().map(Arc::clone),
                    spk: Arc::clone(&processed_spk),
                }),
            };
            if let Err(e) = result {
    ...
    fn recording_mic(frame: &SourceFrame, mode: ChannelMode) -> Arc<[f32]> {
        match mode {
            ChannelMode::SpeakerOnly => Arc::from(vec![0.0; frame.capture.raw_speaker.len()]),
            ChannelMode::MicOnly | ChannelMode::MicAndSpeaker if frame.mic_muted => {
                Arc::from(vec![0.0; frame.capture.raw_mic.len()])
            }
            ChannelMode::MicOnly | ChannelMode::MicAndSpeaker => Arc::clone(&frame.capture.raw_mic),
        }
    }

    fn recording_processed_mic(frame: &SourceFrame, mode: ChannelMode) -> Option<Arc<[f32]>> {
        match mode {
            ChannelMode::SpeakerOnly => None,
            ChannelMode::MicOnly | ChannelMode::MicAndSpeaker if frame.mic_muted => frame
                .capture
                .aec_mic
                .as_ref()
                .map(|mic| Arc::from(vec![0.0; mic.len()])),
            ChannelMode::MicOnly | ChannelMode::MicAndSpeaker => {
                frame.capture.aec_mic.as_ref().map(Arc::clone)
            }
        }
    }
}
...
mod tests {
    ...
    impl poha_storage::StorageRuntime for TestRuntime {
        fn global_base(&self) -> Result<PathBuf, poha_storage::Error> {
            Ok(std::env::temp_dir())
        }

        fn vault_base(&self) -> Result<PathBuf, poha_storage::Error> {
            Ok(std::env::temp_dir())
        }
    }
    ...
    enum ProbeEvent {
        ...
        ListenerDual,
        RecorderSingle,
        RecorderDual(Vec<f32>, Option<Vec<f32>>, Vec<f32>),
    }
            ...
            match message {
                ...
                RecMsg::AudioDual {
                    ...
                    let _ = self.0.send(ProbeEvent::RecorderDual(
                        raw_mic.to_vec(),
                        processed_mic.map(|mic| mic.to_vec()),
                        spk.to_vec(),
                    ));
                }
            }
    ...
    async fn recorder_receives_audio_from_explicit_sink() {
        ...
        let event = tokio::time::timeout(std::time::Duration::from_secs(1), probe_rx.recv())
            ...
            .unwrap()
            .unwrap();
        assert!(matches!(event, ProbeEvent::RecorderDual(_, _, _)));

        handle.abort();
    }

    #[tokio::test]
    async fn recorder_receives_raw_and_processed_mic() {
        let mut pipeline = test_pipeline();
        ...
        let event = tokio::time::timeout(std::time::Duration::from_secs(1), probe_rx.recv())
            ...
            .unwrap()
            .unwrap();
        let ProbeEvent::RecorderDual(raw_mic, processed_mic, spk) = event else {
            panic!("expected recorder dual audio");
        };
        assert_eq!(raw_mic, vec![0.25, -0.25, 0.5, -0.5]);
        assert_eq!(processed_mic, Some(vec![0.1, -0.1, 0.2, -0.2]));
        assert_eq!(spk, vec![0.75, -0.75, 1.0, -1.0]);
    ...
    #[test]
    fn recording_mic_uses_raw_mic_even_when_aec_mic_exists() {
        let frame = source_frame(false);
        let mic = Pipeline::recording_mic(&frame, ChannelMode::MicAndSpeaker);

        assert_eq!(&*mic, &[0.25, -0.25, 0.5, -0.5]);
    }

    #[test]
    fn recording_processed_mic_uses_aec_mic_when_available() {
        let frame = source_frame(false);
        let mic = Pipeline::recording_processed_mic(&frame, ChannelMode::MicAndSpeaker).unwrap();

        assert_eq!(&*mic, &[0.1, -0.1, 0.2, -0.2]);
    }

    #[test]
    fn recording_mic_zeroes_muted_source() {
        let frame = source_frame(true);
        let mic = Pipeline::recording_mic(&frame, ChannelMode::MicAndSpeaker);

        assert_eq!(&*mic, &[0.0, 0.0, 0.0, 0.0]);
    }
}
