...
use poha_audio::{AudioProvider, CaptureConfig, CaptureFrame, CaptureStream};
...
pub(super) async fn start_source_loop(
    ...
    st.audio_ready_emitted = false;

    tracing::info!(?new_mode, mode_changed, "start_source_loop");

    st.pipeline.reset();

    start_streams(myself, st).await
}
...
fn handle_capture_item(
    ctx: &StreamContext,
    item: Option<Result<CaptureFrame, poha_audio::Error>>,
) -> StreamResult {
    match item {
        Some(Ok(frame)) => {
