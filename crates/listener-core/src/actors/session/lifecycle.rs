...
pub(crate) fn configure_sentry_session_context(params: &SessionParams) {
    sentry::configure_scope(|scope| {
        scope.set_tag("poha.session.id", &params.session_id);
        scope.set_tag(
            "poha.session.type",
        ...
        session_context.insert(
            "poha.session.id".to_string(),
            params.session_id.clone().into(),
        );
        session_context.insert(
            "poha.gen_ai.request.model".to_string(),
            params.model.clone().into(),
        );
        session_context.insert(
            "poha.session.transcription_mode".to_string(),
            format!("{:?}", params.transcription_mode).into(),
        );
        session_context.insert(
            "poha.session.onboarding".to_string(),
            params.onboarding.into(),
        );
        session_context.insert(
            "poha.session.language_codes".to_string(),
...
pub(crate) fn clear_sentry_session_context() {
    sentry::configure_scope(|scope| {
        scope.remove_tag("poha.session.id");
        scope.remove_tag("poha.session.type");
        scope.remove_context("poha.session");
    });
}
...
pub(crate) fn emit_session_ended(
    ...
    if let Some(reason) = failure_reason {
        tracing::info!(poha.session.stop_reason = %reason, "session_stopped");
    } else {
