...
use owhisper_client::{
    ...
    PohaAdapter, RealtimeSttAdapter, SonioxAdapter, poha_ws_client,
};
...
fn desktop_connect_policy() -> poha_ws_client::client::WebSocketConnectPolicy {
    poha_ws_client::client::WebSocketConnectPolicy {
...
> {
    ...
    let client = owhisper_client::ListenClient::builder()
        ...
        .extra_header(DEVICE_FINGERPRINT_HEADER, poha_host::fingerprint())
        .build_single()
        .await;
    ...
    let (listen_stream, handle) = match client.from_realtime_audio(outbound).await {
        Err(e) => {
            tracing::error!(
                poha.session.id = %args.session_id,
                error.message = ?e,
...
mod tests {
    ...
    impl poha_storage::StorageRuntime for NoopRuntime {
