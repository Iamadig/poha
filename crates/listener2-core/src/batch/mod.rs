...
impl BatchProvider {
    pub fn to_adapter_kind(&self) -> Option<AdapterKind> {
        match self {
            ...
            Self::Poha => Some(AdapterKind::Poha),
...
pub struct BatchParams {
    ...
    pub api_key: String,
    #[serde(default)]
    pub languages: Vec<poha_language::Language>,
    #[serde(default)]
...
async fn run_batch_inner(
    ...
    let metadata_joined = tokio::task::spawn_blocking({
        ...
        move || poha_audio_utils::audio_file_metadata(path)
    })
    .await;
    ...
    let metadata = match metadata_result {
        ...
        Err(err) => {
            ...
            tracing::error!(
                error = %raw_error,
                poha.error.user_message = %message,
...
pub(super) fn session_span(session_id: &str) -> tracing::Span {
    tracing::info_span!("session", poha.session.id = %session_id)
}
