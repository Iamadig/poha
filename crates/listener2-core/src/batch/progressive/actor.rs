            ...
            result = tokio::time::timeout(
                ...
                match result {
                    Ok(Some(Ok(item))) => {
                        ...
                        {
                            tracing::error!(
                                poha.stt.provider.name = %provider,
                                error.code = ?error_code,
                                error = %error_message,
                                poha.response.count = response_count,
                    ...
                    Ok(None) => {
                        ...
                        tracing::error!(
                            poha.response.count = response_count,
                            poha.completions.expected = expected_completions,
                            poha.completions.seen = completions_seen,
