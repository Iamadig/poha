...
fn run_denoise_blocking(
    ...
    let metadata = poha_audio_utils::audio_file_metadata(&params.input_path)
        .map_err(|e| crate::Error::DenoiseError(e.to_string()))?;
    let channels = metadata.channels.max(1) as usize;

    let source = poha_audio_utils::source_from_path(&params.input_path)
        .map_err(|e| crate::Error::DenoiseError(e.to_string()))?;

    let samples = poha_audio_utils::resample_audio(source, DENOISE_SAMPLE_RATE)
        .map_err(|e| crate::Error::DenoiseError(e.to_string()))?;

    let channel_buffers = poha_audio_utils::deinterleave(&samples, channels);

    let mut denoisers: Vec<poha_denoise::onnx::Denoiser> = (0..channels)
        .map(|_| poha_denoise::onnx::Denoiser::new())
        .collect::<Result<_, _>>()
        .map_err(|e| crate::Error::DenoiseError(e.to_string()))?;
    ...
    let output = poha_audio_utils::interleave(&output_channels);
