...
use poha_audio_interface::AsyncSource;
...
impl SpeakerInput {
    pub fn new() -> Result<Self> {
        tracing::info!("speaker_input_new_start");
        let tap_desc = ca::TapDesc::with_stereo_global_tap_excluding_processes(&ns::Array::new());
        tracing::info!("speaker_tap_desc_created");
        let tap = tap_desc.create_process_tap()?;
        tracing::info!(tap_uid = ?tap.uid(), "speaker_process_tap_created");
        ...
        tracing::info!("speaker_input_new_done");
        ...
        extern "C" fn proc(
            ...
            match ctx.common_format {
                ...
                av::audio::CommonFormat::PcmF64 => {
                    process_samples_rt_safe::<f64>(ctx, first_buffer);
                }
                av::audio::CommonFormat::PcmI32 => {
                    process_samples_rt_safe::<i32>(ctx, first_buffer);
                }
                av::audio::CommonFormat::PcmI16 => {
                    process_samples_rt_safe::<i16>(ctx, first_buffer);
                }
                _ => {}
            }
...
fn process_samples_rt_safe<T>(ctx: &mut Ctx, buffer: &cat::AudioBuf)
where
    T: Copy + ToSample<f32> + 'static,
{
...
mod tests {
    ...
    #[test]
    fn macos_speaker_path_downmixes_interleaved_stereo() {
        ...
        let mut ctx = Ctx {
            ...
            conversion_buffer: vec![0.0; 8],
        };

        process_audio_data_rt_safe(&mut ctx, &[0.0, 0.2, 0.4, 0.6]);
