        ...
        .param_changed(|_, user_data, id, param| {
            ...
            if user_data.format.parse(param).is_ok() {
                ...
                if rate > 0 {
                    ...
                    tracing::info!(
                        poha.audio.sample_rate_hz = rate,
...
fn pulseaudio_capture_loop(
    ...
    let mut context = PaContext::new(&mainloop, "poha-speaker-capture")
    ...
    let setup_result = (|| -> Result<_> {
        ...
        tracing::info!(poha.audio.device = %monitor_device, "connecting_to_monitor_device");

        mainloop.lock();
        let stream_result = (|| -> Result<_> {
            let mut stream = PaStream::new(&mut context, "poha-capture", &spec, None)
    ...
    tracing::info!(
        poha.audio.sample_rate_hz = actual_rate,
