...
#[pin_project]
pub struct NormalizedSource<S: poha_audio_interface::AsyncSource> {
    source: S,
    gain_linear: f32,
    ebur128: EbuR128,
...
pub trait NormalizeExt<S: poha_audio_interface::AsyncSource> {
    fn normalize(self) -> NormalizedSource<S>;
}

impl<S: poha_audio_interface::AsyncSource> NormalizeExt<S> for S {
    fn normalize(self) -> NormalizedSource<S> {
...
impl<S: poha_audio_interface::AsyncSource> Stream for NormalizedSource<S> {
    type Item = f32;
...
impl<S: poha_audio_interface::AsyncSource> poha_audio_interface::AsyncSource
...
mod tests {
    ...
    async fn test_normalize() {
        let audio = rodio::Decoder::new(std::io::BufReader::new(
            std::fs::File::open(poha_data::english_1::AUDIO_PATH).unwrap(),
        ))
        .unwrap();
