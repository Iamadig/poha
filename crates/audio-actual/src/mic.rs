...
impl MicInput {
    ...
    pub fn new(device_name: Option<String>) -> Result<Self, crate::Error> {
        ...
        let device = match (device_name.as_deref(), default_input_device) {
            (None, Some(device)) => device,
            (Some(selected), Some(device)) if selected == get_device_name(&device) => device,
            _ => {
                ...
                device_name
                    ...
                    .or_else(|| input_devices.into_iter().next().map(|(_, d)| d))
        ...
        tracing::info!(
            poha.audio.sample_rate_hz = ?config.sample_rate(),
            device_name,
