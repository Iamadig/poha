...
fn stt_model() -> Model {
    ...
    let default = format!(
        "{}/Library/Application Support/com.iamadig.poha.dev/models/cactus/whisper-small-int8-apple",
        home
    );
...
fn test_transcribe_file() {
    ...
    let r = model
        .transcribe_file(poha_data::english_1::AUDIO_PATH, &options)
        .unwrap();
...
fn test_transcribe_file_with_callback() {
    ...
    let r = model
        .transcribe_file_with_callback(poha_data::english_1::AUDIO_PATH, &options, |token| {
...
fn test_transcribe_pcm() {
    ...
    let r = model
        .transcribe_pcm(poha_data::english_1::AUDIO, &options)
        .unwrap();
...
fn test_stream_transcriber() {
    let model = stt_model();
    let pcm = poha_data::english_1::AUDIO;
    let options = en_options();
