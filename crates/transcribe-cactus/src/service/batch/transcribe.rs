...
use poha_audio_utils::content_type_to_extension;
use poha_transcribe_core::{
...
#[tracing::instrument(
    ...
    fields(
        poha.audio.size_bytes = audio_data.len(),
        poha.file.mime_type = content_type,
        poha.model.path = %model_path.display()
    )
)]
pub(super) fn transcribe_batch(
    ...
    let source = poha_audio_utils::source_from_path(temp_file.path())?;
    let channel_count = u16::from(source.channels()).max(1) as usize;
    let resampled = poha_audio_utils::resample_audio(source, TARGET_SAMPLE_RATE)?;
...
fn transcribe_chunks(
    channel_idx: usize,
    chunks: &[poha_audio_chunking::AudioChunk],
    channel_duration: f64,
    model: &poha_cactus::Model,
    options: &poha_cactus::TranscribeOptions,
    ...
    for (chunk_idx, chunk) in chunks.iter().enumerate() {
        let pcm_i16 = poha_audio_utils::f32_to_i16_samples(&chunk.samples);
...
mod tests {
    ...
    fn initial_resolved_until_uses_leading_silence() {
        let chunks = vec![poha_audio_chunking::AudioChunk {
            samples: vec![],
    ...
    fn e2e_transcribe_with_real_model_inference() {
        let model_path_str = std::env::var("CACTUS_STT_MODEL").unwrap_or_else(|_| {
            dirs::data_dir()
                ...
                .join("com.iamadig.poha.dev/models/cactus/whisper-small-int8-apple")
                .to_string_lossy()
                .into_owned()
        });
        ...
        let wav_bytes = std::fs::read(poha_data::english_1::AUDIO_PATH)
        ...
        let model = poha_cactus::Model::new(model_path)
