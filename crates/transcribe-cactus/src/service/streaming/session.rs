...
use poha_model_manager::ModelManager;
use poha_ws_utils::ConnectionGuard;
...
type TaggedEvent = (
    usize,
    Result<poha_cactus::TranscribeEvent, poha_cactus::Error>,
);
...
pub(super) async fn handle_websocket(
    socket: WebSocket,
    params: ListenParams,
    model: Arc<poha_cactus::Model>,
    metadata: Metadata,
    cactus_config: crate::CactusConfig,
    guard: ConnectionGuard,
    manager: ModelManager<poha_cactus::Model>,
) {
    ...
    for ch_idx in 0..total_channels {
        ...
        let (audio_tx, session) = poha_cactus::transcribe_stream(
            model.clone(),
            options.clone(),
            cloud_config,
            chunk_size_ms,
...
impl Session {
    async fn handle_transcribe_event(&mut self, event: Option<TaggedEvent>) -> LoopAction {
        ...
        match event {
            ...
            Ok(poha_cactus::TranscribeEvent {
                result,
                chunk_duration_secs,
            }) => {
    ...
    async fn process_result(
        &mut self,
        ch_idx: usize,
        result: poha_cactus::StreamResult,
        ...
        if result.cloud_result_job_id != 0 && !result.cloud_result.is_empty() {
            ...
            tracing::info!(
                poha.transcript.char_count = cloud_text.chars().count() as u64,
                poha.stt.job.id = job_id,
                poha.audio.channel_index = ch_idx,
        ...
        if !confirmed_text.is_empty() && confirmed_text != state.last_confirmed_sent {
            ...
            tracing::info!(
                poha.transcript.char_count = confirmed_text.chars().count() as u64,
                poha.audio.channel_index = ch_idx,
