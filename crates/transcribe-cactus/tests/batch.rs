...
fn audio_wav_bytes() -> Vec<u8> {
    std::fs::read(poha_data::english_1::AUDIO_PATH).expect("failed to read audio file")
}
...
async fn health_starts_loading_then_fails_for_invalid_model_path() {
    ...
    let body = to_bytes(response.into_body(), 1024 * 1024).await.unwrap();
    let health: poha_cactus_model::CactusServiceHealth = serde_json::from_slice(&body).unwrap();
    assert_eq!(
        health.status,
        poha_cactus_model::CactusServiceStatus::Loading
    );
    ...
    assert_eq!(
        health.status,
        poha_cactus_model::CactusServiceStatus::Failed
    );
    assert!(
        health
            .error
