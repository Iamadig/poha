...
use poha_cactus::CloudConfig;
use transcribe_cactus::CactusConfig;

fn load_english_10_mono_pcm() -> Vec<u8> {
    let mic = poha_audio_utils::source_from_path(poha_data::english_10::AUDIO_MIC_MP3_PATH)
        .expect("failed to open mic audio");
    let mic_f32 = poha_audio_utils::resample_audio(mic, 16_000).expect("failed to resample mic");

    let spk = poha_audio_utils::source_from_path(poha_data::english_10::AUDIO_SPK_MP3_PATH)
        .expect("failed to open spk audio");
    let spk_f32 = poha_audio_utils::resample_audio(spk, 16_000).expect("failed to resample spk");

    let mixed = poha_audio_utils::mix_audio_f32(&mic_f32, &spk_f32);

    // First 60 seconds (16000 samples/s × 60s)
    let limit = 16_000 * 60;
    let samples = &mixed[..mixed.len().min(limit)];
    poha_audio_utils::f32_to_i16_bytes(samples.iter().copied()).to_vec()
}
