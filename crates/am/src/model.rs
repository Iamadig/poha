...
impl AmModel {
    ...
    pub fn supported_languages(&self) -> Vec<poha_language::Language> {
        ...
        match self {
            ...
            AmModel::ParakeetV3 => PARAKEET_V3_LANGS
                ...
                .map(|iso| iso.into())
                .collect(),
            AmModel::WhisperLargeV3 => poha_language::whisper_multilingual(),
        }
    }
    ...
    pub fn tar_url(&self) -> Option<&str> {
        None
    }
    ...
    pub async fn download<F: Fn(poha_download_interface::DownloadProgress) + Send + Sync>(
        &self,
        output_path: impl AsRef<std::path::Path>,
        progress_callback: F,
    ) -> Result<(), crate::Error> {
        let tar_url = self
            .tar_url()
            .ok_or(crate::Error::ModelDownloadUnavailable)?;
        poha_file::download_file_parallel(tar_url, output_path, progress_callback).await?;
        Ok(())
    }
}
