...
impl CactusSttModel {
    ...
    pub fn supported_languages(&self) -> Vec<poha_language::Language> {
        match self {
            ...
            _ => poha_language::whisper_multilingual(),
        }
    }
}
    ...
    pub fn source(&self) -> CactusModelSource {
        match self {
            CactusLlmModel::Lfm2Vl450mApple => CactusModelSource::BundledResource {
                relative_path: "models/cactus/poha-vlm/weight",
            },
...
mod tests {
    ...
    fn downloadable_models_keep_existing_metadata() {
        assert_eq!(CactusSttModel::WhisperSmallInt4.model_url(), None);
        assert_eq!(CactusSttModel::WhisperSmallInt4.checksum(), None);
    }
}
