...
pub use poha_am::AmModel;
pub use poha_cactus_model::{CactusLlmModel, CactusModel, CactusModelSource, CactusSttModel};
use poha_model_downloader::{DownloadableModel, Error, extract_zip};
pub use poha_whisper_local_model::WhisperModel;
...
impl LocalModel {
    pub fn all() -> Vec<LocalModel> {
        ...
        models.extend(
            CactusSttModel::all()
                .iter()
                .filter(|model| CactusModel::Stt((*model).clone()).model_url().is_some())
                .cloned()
...
impl DownloadableModel for GgufLlmModel {
    ...
    fn is_downloaded(&self, models_base: &Path) -> Result<bool, Error> {
        ...
        let actual =
            poha_file::file_size(&path).map_err(|e| Error::OperationFailed(e.to_string()))?;
...
impl DownloadableModel for LocalModel {
    ...
    fn download_url(&self) -> Option<String> {
        match self {
            ...
            LocalModel::Am(model) => model.tar_url().map(ToString::to_string),
    ...
    fn download_checksum(&self) -> Option<u32> {
        match self {
            ...
            LocalModel::Am(model) => model.tar_url().map(|_| model.tar_checksum()),
