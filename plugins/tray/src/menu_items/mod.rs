...
macro_rules! menu_items {
    ($($variant:ident => $item:ty),* $(,)?) => {
        ...
        pub enum PohaMenuItem {
            $($variant),*
        }

        impl From<PohaMenuItem> for tauri::menu::MenuId {
            fn from(value: PohaMenuItem) -> Self {
                match value {
                    $(PohaMenuItem::$variant => <$item as MenuItemHandler>::ID),*
                }.into()
            }
        }

        impl TryFrom<tauri::menu::MenuId> for PohaMenuItem {
            type Error = ();

            fn try_from(id: tauri::menu::MenuId) -> std::result::Result<Self, Self::Error> {
                let id = id.0.as_str();
                match id {
                    $(<$item as MenuItemHandler>::ID => Ok(PohaMenuItem::$variant),)*
                    _ => Err(()),
                }
            }
        }
        ...
        impl PohaMenuItem {
            pub fn handle(self, app: &AppHandle<tauri::Wry>) {
                match self {
                    $(PohaMenuItem::$variant => <$item>::handle(app)),*
                }
            }
        }
    };
