...
use poha_model_downloader::{ModelDownloadManager, ModelDownloaderRuntime};
...
impl<R: Runtime> ModelDownloaderRuntime<LocalModel> for TauriModelRuntime<R> {
    fn models_base(&self) -> Result<PathBuf, poha_model_downloader::Error> {
    ...
    fn emit_progress(&self, model: &LocalModel, status: poha_model_downloader::DownloadStatus) {
    ...
    pub async fn start_server(&self, model: LocalModel) -> Result<String, crate::Error> {
        ...
        match server_type {
            ServerType::Internal => {
                #[cfg(target_arch = "aarch64")]
                {
                    use poha_transcribe_cactus::CactusConfig;
...
async fn start_internal2_server(
    ...
    cache_dir: PathBuf,
    model: poha_cactus_model::CactusSttModel,
    cactus_config: poha_transcribe_cactus::CactusConfig,
...
async fn start_internal_server(
    supervisor: &supervisor::SupervisorRef,
    cache_dir: PathBuf,
    model: poha_whisper_local_model::WhisperModel,
) -> Result<String, crate::Error> {
