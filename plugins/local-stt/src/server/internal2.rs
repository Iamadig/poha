...
use poha_cactus_model::{CactusServiceHealth, CactusServiceStatus, CactusSttModel};
...
pub struct Internal2STTArgs {
    ...
    pub cactus_config: poha_transcribe_cactus::CactusConfig,
}
...
impl Actor for Internal2STTActor {
    ...
    async fn pre_start(
        ...
        let router = poha_transcribe_cactus::TranscribeService::builder()
        ...
        match message {
            ...
            Internal2STTMessage::GetHealth(reply_port) => {
                let health_url = format!(
                    "http://{}{}",
                    state.server_addr,
                    poha_transcribe_cactus::HEALTH_PATH,
                );
                ...
                let info = ServerInfo {
                    url: Some(format!(
                        "http://{}{}",
                        state.server_addr,
                        poha_transcribe_cactus::LISTEN_PATH,
                    )),
                    status,
