...
fn cleanup_state(state: &mut ExternalSTTState) {
    ...
    if kill_failed {
        poha_host::kill_processes_by_matcher(poha_host::ProcessMatcher::Sidecar);
    }
...
impl Actor for ExternalSTTActor {
    ...
    async fn pre_start(
        ...
        let client = poha_am::Client::new(&base_url);
    ...
    async fn post_start(
        ...
        let res = (|| async {
            let response = state
                .client
                .init(
                    poha_am::InitRequest::new(api_key.clone())
                        .with_model(model.clone(), &models_dir),
                )
                .await?;

            if !response.is_success() {
                return Err(poha_am::Error::ServerError {
        ...
        match message {
            ...
            ExternalSTTMessage::GetHealth(reply_port) => {
                let status = match state.client.status().await {
                    Ok(r) => match r.status {
                        poha_am::ServerStatusType::Ready => ServerStatus::Ready,
                        poha_am::ServerStatusType::Initializing => ServerStatus::Loading,
