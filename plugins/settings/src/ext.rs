...
use poha_storage::ObsidianVault;
...
impl<'a, R: tauri::Runtime, M: tauri::Manager<R>> Settings<'a, R, M> {
    fn settings_base_path(&self) -> Result<PathBuf, crate::Error> {
        ...
        let path = poha_storage::global::compute_default_base(bundle_id)
            .ok_or(poha_storage::Error::DataDirUnavailable)?;
    ...
    pub fn settings_base(&self) -> Result<Utf8PathBuf, crate::Error> {
        ...
        Utf8PathBuf::from_path_buf(path).map_err(|_| poha_storage::Error::PathNotValidUtf8.into())
    }
    ...
    pub fn settings_path(&self) -> Result<Utf8PathBuf, crate::Error> {
        ...
        Ok(base.join(poha_storage::vault::SETTINGS_FILENAME))
    }
    ...
    pub fn obsidian_vaults(&self) -> Result<Vec<ObsidianVault>, crate::Error> {
        poha_storage::obsidian::list_vaults().map_err(Into::into)
    }
...
impl<'a, R: tauri::Runtime, M: tauri::Manager<R>> Settings<'a, R, M> {
    pub async fn copy_vault(&self, new_path: Utf8PathBuf) -> Result<(), crate::Error> {
        ...
        poha_storage::vault::validate_vault_base_change(
            old_vault_base.as_ref(),
            new_path.as_ref(),
        )?;
        poha_storage::vault::ensure_vault_dir(new_path.as_ref())?;
        poha_storage::vault::fs::copy_vault_items(old_vault_base.as_ref(), new_path.as_ref())
            .await?;
    ...
    pub async fn move_vault(&self, new_path: Utf8PathBuf) -> Result<(), crate::Error> {
        ...
        if !poha_storage::vault::fs::is_empty_or_missing_dir(new_path.as_ref())? {
            return Err(poha_storage::Error::VaultBaseIsNotEmpty.into());
        }
        poha_storage::vault::ensure_vault_dir(new_path.as_ref())?;
        ...
        let _ = poha_storage::vault::fs::remove_vault_items(old_vault_base.as_ref()).await;
    ...
    pub async fn set_vault_base(&self, new_path: Utf8PathBuf) -> Result<(), crate::Error> {
        let settings_base = self.settings_base_path()?;
        poha_storage::vault::persist_vault_path(&settings_base, &settings_base, new_path.as_ref())?;
        Ok(())
    }
}
