...
impl StartupSnapshot {
    ...
    fn settings_path(&self) -> PathBuf {
        poha_storage::vault::compute_settings_path(&self.startup_vault_base)
    }
    ...
    pub async fn save(&self, settings: serde_json::Value) -> crate::Result<()> {
        ...
        poha_storage::fs::atomic_write_async(&self.settings_path(), &content).await?;
        Ok(())
    }

    pub fn reset(&self) -> crate::Result<()> {
        poha_storage::fs::atomic_write(&self.settings_path(), "{}")?;
        Ok(())
    }
}
