...
pub use poha_analytics::*;

pub type ManagedState = poha_analytics::AnalyticsClient;
...
pub fn init<R: tauri::Runtime>() -> tauri::plugin::TauriPlugin<R> {
    ...
    tauri::plugin::Builder::new(PLUGIN_NAME)
        ...
        .setup(|app, _api| {
            let posthog_key = option_env!("POSTHOG_API_KEY").filter(|v| v.starts_with("phc_"));

            let client = {
                let mut builder = poha_analytics::AnalyticsClientBuilder::default();
...
mod test {
    ...
    fn create_app<R: tauri::Runtime>(builder: tauri::Builder<R>) -> tauri::App<R> {
        ...
        ctx.config_mut().identifier = "com.iamadig.poha.dev".to_string();
    ...
    async fn test_analytics() {
        ...
        let result = app
            .analytics()
            .event(poha_analytics::AnalyticsPayload::builder("test_event").build())
            .await;
        assert!(result.is_ok());
