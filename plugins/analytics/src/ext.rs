...
impl<'a, R: tauri::Runtime, M: tauri::Manager<R>> Analytics<'a, R, M> {
    pub async fn event(
        &self,
        mut payload: poha_analytics::AnalyticsPayload,
    ) -> Result<(), crate::Error> {
        ...
        let machine_id = poha_host::fingerprint();
        let client = self.manager.state::<crate::ManagedState>();
        client
            .event(machine_id, payload)
            .await
            .map_err(crate::Error::PohaAnalytics)?;

        Ok(())
    }

    pub fn event_fire_and_forget(&self, mut payload: poha_analytics::AnalyticsPayload) {
        Self::enrich_payload(self.manager, &mut payload);
    ...
    fn enrich_payload(manager: &M, payload: &mut poha_analytics::AnalyticsPayload) {
    ...
    pub async fn set_properties(
        &self,
        payload: poha_analytics::PropertiesPayload,
    ) -> Result<(), crate::Error> {
        if !self.is_disabled()? {
            let machine_id = poha_host::fingerprint();
