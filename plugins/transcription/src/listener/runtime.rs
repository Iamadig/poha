use poha_transcription_core::listener::ListenerRuntime;
...
use poha_transcription_core::listener::State as RootState;
use poha_transcription_core::listener::actors::{RootActor, RootMsg};
...
impl poha_storage::StorageRuntime for TauriRuntime {
    fn global_base(&self) -> Result<std::path::PathBuf, poha_storage::Error> {
        self.app
            .settings()
            .global_base()
            .map(|p| p.into_std_path_buf())
            .map_err(|_| poha_storage::Error::DataDirUnavailable)
    }

    fn vault_base(&self) -> Result<std::path::PathBuf, poha_storage::Error> {
        self.app
            .settings()
            .vault_base()
            .map(|p| p.into_std_path_buf())
...
impl ListenerRuntime for TauriRuntime {
    fn emit_lifecycle(&self, event: poha_transcription_core::listener::SessionLifecycleEvent) {
        use tauri_plugin_tray::TrayPluginExt;
        match &event {
            poha_transcription_core::listener::SessionLifecycleEvent::Active { error, .. } => {
            ...
            poha_transcription_core::listener::SessionLifecycleEvent::Inactive { .. } => {
            ...
            poha_transcription_core::listener::SessionLifecycleEvent::Finalizing { .. } => {}
        }

        let capture_event = match event {
            poha_transcription_core::listener::SessionLifecycleEvent::Active {
                session_id,
                ...
                let requested_live_transcription = requested_transcription_mode
                    == poha_transcription_core::listener::TranscriptionMode::Live;
            ...
            poha_transcription_core::listener::SessionLifecycleEvent::Finalizing { session_id } => {
    ...
    fn emit_progress(&self, event: poha_transcription_core::listener::SessionProgressEvent) {
        if let Err(error) = CaptureStatusEvent::from(event).emit(&self.app) {
            tracing::error!(?error, "failed_to_emit_progress_event");
        }
    }

    fn emit_error(&self, event: poha_transcription_core::listener::SessionErrorEvent) {
        if let Err(error) = CaptureStatusEvent::from(event).emit(&self.app) {
            tracing::error!(?error, "failed_to_emit_error_event");
        }
    }

    fn emit_data(&self, event: poha_transcription_core::listener::SessionDataEvent) {
