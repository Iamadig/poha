...
use poha_transcript::{RenderTranscriptRequest, RenderedTranscriptSegment};
...
pub async fn is_supported_languages_live<R: tauri::Runtime>(
    ...
    let languages_parsed = languages
        .iter()
        .map(|s| poha_language::Language::from_str(s))
...
pub async fn render_transcript_segments(
    ...
    Ok(poha_transcript::render_transcript_segments(params))
}
